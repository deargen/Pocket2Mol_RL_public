...
class MessageModule(Module):
    def __init__(
        self, node_sca, node_vec, edge_sca, edge_vec, out_sca, out_vec, cutoff=10.0
    ):
        super().__init__()
    ...
    def forward(
        ...
        node_scalar, node_vector = (
            node_scalar[edge_index_node],
            node_vector[edge_index_node],
        )
        ...
        y_edge_vector = self.n2e_linear(node_scalar).unsqueeze(-1) * self.edge_vnlinear(
            edge_vector
        )
...
class GVLinear(Module):
    def __init__(self, in_scalar, in_vector, out_scalar, out_vector):
        super().__init__()
        dim_hid = max(in_vector, 1 if out_vector is None else out_vector)
    ...
    def forward(self, features):
        ...
        if self.out_vector_dim is not None:
            out_vector = self.lin_vector2(feat_vector_inter)

        if self.out_vector_dim is not None:
            gating = torch.sigmoid(self.scalar_to_vector_gates(out_scalar)).unsqueeze(
                dim=-1
            )
...
class VNLeakyReLU(nn.Module):
    ...
    def forward(self, x):
        ...
        x: point features of shape [B, N_samples, N_feat, 3]
        """
        d = self.map_to_dir(x.transpose(-2, -1)).transpose(
            -2, -1
        )  # (N_samples, N_feat, 3)
