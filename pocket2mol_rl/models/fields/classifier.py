...
class SpatialClassiferVNSkipEdge(Module):
    ...
    def load_params_from(self, model: "SpatialClassifierVN"):
        if type(model) != SpatialClassifierVN:
            raise NotImplementedError()

        self.message_module.load_state_dict(model.message_module.state_dict())
        self.classifier.load_state_dict(model.classifier.state_dict())
        self.distance_expansion.load_state_dict(model.distance_expansion.state_dict())
        self.vector_expansion.load_state_dict(model.vector_expansion.state_dict())
    ...
    def forward(
        ...
        """
        Args:
            pos_query:   (N_query, space_dim)
            edge_index_query: (2, N_q_c, )
            pos_ctx:     (N_ctx, space_dim)
        ...
        vec_ij = (
            pos_query[edge_index_q_cps_knn[0]] - pos_compose[edge_index_q_cps_knn[1]]
        )
        ...
        y = [
            scatter_add(
                h[0], index=edge_index_q_cps_knn[0], dim=0, dim_size=pos_query.size(0)
            ),  # (N_query, F)
            scatter_add(
                h[1], index=edge_index_q_cps_knn[0], dim=0, dim_size=pos_query.size(0)
            ),
        ]
...
class SpatialClassifierVN(Module):
    def __init__(
        ...
        self.edge_atten = AttentionEdges(
            num_filters, num_filters, num_heads, num_edge_types, space_dim=space_dim
        )
        self.edge_pred = GVLinear(num_filters[0], num_filters[1], num_edge_types, 1)

        self.distance_expansion = GaussianSmearing(
            stop=cutoff, num_gaussians=edge_channels
        )
        self.distance_expansion_3A = GaussianSmearing(
            stop=3.0, num_gaussians=edge_channels
        )
        self.vector_expansion = EdgeExpansion(
            edge_channels
        )  # Linear(in_features=1, out_features=edge_channels, bias=False)
    ...
    def forward(
        ...
        if (len(edge_index_query) != 0) and (edge_index_query.size(1) > 0):
            ...
            edge_attr = (
                torch.cat(
                    [node_mol_i[0], node_mol_j[0], edge_feat[0]], dim=-1
                ),  # (E, F)
            ...
            edge_pred = torch.empty([0, self.num_edge_types], device=pos_query.device)
...
class AttentionEdges(Module):
    def __init__(
        self, hidden_channels, key_channels, num_heads, num_edge_types, space_dim
    ):
        super().__init__()

        assert (hidden_channels[0] % num_heads == 0) and (
            hidden_channels[1] % num_heads == 0
        )
        ...
        self.atten_bias_lin = AttentionBias(
            self.num_heads, hidden_channels, num_edge_types
        )
        self.layernorm_sca = LayerNorm([hidden_channels[0]])
        self.layernorm_vec = LayerNorm([hidden_channels[1], space_dim])
    ...
    def forward(
        ...
        """
        Args:
            x:  edge features: scalar features (N, feat), vector features(N, feat, space_dim)
            edge_attr:  (E, H)
        ...
        h_queries = (
            ...
            h_queries[1].view(N, self.num_heads, -1, self.space_dim),
        )  # (N, heads, K_per_head, space_dim)
        h_keys = self.k_lin(edge_attr)
        h_keys = (
            h_keys[0].view(N, self.num_heads, -1),  # (N, heads, K_per_head)
            h_keys[1].view(N, self.num_heads, -1, self.space_dim),
        )  # (N, heads, K_per_head, space_dim)
        h_values = self.v_lin(edge_attr)
        h_values = (
            h_values[0].view(N, self.num_heads, -1),  # (N, heads, K_per_head)
            h_values[1].view(N, self.num_heads, -1, self.space_dim),
        )  # (N, heads, K_per_head, space_dim)
        ...
        output = [
            ...
            scatter_sum(
                (alpha[1].unsqueeze(-1).unsqueeze(-1) * values_j[1]).view(
                    num_attens, -1, self.space_dim
                ),
                index_edge_i_list,
                dim=0,
                dim_size=N,
...
class AttentionBias(Module):
    def __init__(
        self, num_heads, hidden_channels, num_edge_types, cutoff=10.0
