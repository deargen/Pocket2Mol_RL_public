...
class AttentionInteractionBlockVN(Module):
    ...
    def forward(self, x, edge_index, edge_feature, edge_vector):
        ...
        edge_sca_feat = torch.cat(
            [self.distance_expansion(edge_dist), edge_feature], dim=-1
        )
        ...
        # Aggregate messages
        aggr_msg_sca = scatter_sum(
            msg_j_sca, row, dim=0, dim_size=N
        )  # .view(N, -1) # (N, heads*H_per_head)
        aggr_msg_vec = scatter_sum(
            msg_j_vec, row, dim=0, dim_size=N
        )  # .view(N, -1, 3) # (N, heads*H_per_head, 3)
