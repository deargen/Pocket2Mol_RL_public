...
class CheckGeometryException(Exception):
    pass
...
def check_geometry(
    ...
    if mol.GetNumConformers() == 0:
        msg = "Molecule does not have a conformer."
        if raise_error:
            raise CheckGeometryException(msg)
        logger.warning(msg)
        return {"results": results}

    if mol.GetNumAtoms() == 1:
        msg = f"Molecule has only {mol.GetNumAtoms()} atoms."
        if raise_error:
            raise CheckGeometryException(msg)
        logger.warning(msg)
    ...
    results[col_n_good_bonds] = (
        results[col_n_bonds] - results[col_n_short_bonds] - results[col_n_long_bonds]
    )
    ...
    return {
        "results": results,
        "details": {"bonds": df_bonds, "clash": df_clash, "angles": df_angles},
    }
