...
class CheckEnergyRatioException(Exception):
    pass
...
def check_energy_ratio(
    ...
    try:
        ...
        if raise_error:
            raise CheckEnergyRatioException() from e
        logger.warning(
            "Failed to calculate ensemble conformation energy for %s: %s", inchi, e
        )
        avg_energy = np.nan
...
def new_conformation(
    ...
    with CaptureLogger():
        ...
        if cids_etkdg is None or len(cids_etkdg) != n_confs:
            msg = "Failed to generate conformations."
            raise CheckEnergyRatioException(msg)
