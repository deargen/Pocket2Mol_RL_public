...
from rdkit.Chem.rdMolAlign import GetAlignmentTransform  # GetBestAlignmentTransform
...
def remove_all_charges_and_hydrogens(mol: Mol) -> Mol:
    ...
    try:
        ...
        logger.error(
            "AtomValenceException raised while neutralizing molecule. Continuing with original molecule."
        )
        return mol
    return mol
...
def add_stereo_hydrogens(mol: Mol) -> Mol:
    ...
    exclude = {
        match[1] for match in mol.GetSubstructMatches(MolFromSmarts("[CX3]=[NH1]"))
    }
    atoms = [
        a.GetIdx()
        for a in mol.GetAtoms()
        if a.GetAtomicNum() != 1
        if a.GetIdx() not in exclude
    ]
...
def _align_and_renumber(mol_true: Mol, mol_pred: Mol) -> Mol:
    ...
    alignment = GetAlignmentTransform(
        mol_true, mol_pred, symmetrizeConjugatedTerminalGroups=False
    )
...
def _renumber_mol1_to_match_mol2(mol1: Mol, mol2: Mol) -> Mol:
    ...
    rmsd, transform, atom_map = GetAlignmentTransform(mol1, mol2, reflect=False)
