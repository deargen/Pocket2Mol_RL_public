...
class ProteinLigandData(Data):
    ...
    @property
    def ligand_context_size(self) -> int:
    ...
    def __inc__(self, key, value, *args, **kwargs):
        if key == "ligand_bond_index":
            ...
            key == "real_compose_knn_edge_index_0"
        ):  # edges for message passing of  field
            return self["pos_real"].size(0)
        elif key == "fake_compose_knn_edge_index_0":
            return self["pos_fake"].size(0)
        elif (key == "real_compose_knn_edge_index_1") or (
            key == "fake_compose_knn_edge_index_1"
        ):
            return self["compose_pos"].size(0)
